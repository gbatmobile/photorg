...
def filterDateRange(allFiles):
    if len(dateRange) == 2:
        log ("STATUS: Filtering files in date range ...")
        allFiles = list(filter(lambda file: dateRange[0] < file['date']
                                                         < dateRange[1],
                               allFiles))
    return allFiles
...
def showHelp(msg=''):
    ...
    print ("\t--usage - Show examples of usage\n")
    ...
    print ("\t-s YYYY-MM-DD YYYY-MM-DD - organize file only in date range")
...
def processOptions():
    global  organizeBy, importEvents, eventFile, sourceDir, dateRange
    ...
    try:
        ...
        while nextArg < len(sys.argv):
            if sys.argv[nextArg] in organizeOptions:
                ...
                onlyPhotos = True
            elif sys.argv[nextArg] == '-s' and (nextArg+2) < len(sys.argv):
                dateRange = []
                dateRange.append(datetime.datetime.strptime(sys.argv[nextArg+1], "%Y-%m-%d"))
                dateRange.append(datetime.datetime.strptime(sys.argv[nextArg+2], "%Y-%m-%d"))
                dateRange[1] += datetime.timedelta(seconds=86399)
                nextArg += 2
                ...
                nextArg += 1
            elif sys.argv[nextArg] == '--usage':
                showExamplesAndExit()
            elif sys.argv[nextArg] == '--help':
                showHelpAndExit()
            else:
                showHelp("ERROR: Invalid option: "+sys.argv[nextArg])
                showExamplesAndExit()
            nextArg += 1
        ...
        showHelp(str(e)+" current arg: "+sys.argv[nextArg])
        showExamplesAndExit()
...
if __name__ == "__main__":
    ...
    addType(allFilesSrc)
    addDate(allFilesSrc)
    allFilesSrc = filterDateRange(allFilesSrc)
