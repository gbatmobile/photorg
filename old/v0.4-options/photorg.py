...
def locateAllFiles(dirs):
    global events
    log ("Locating files in "+str(dirs)+" ...")
    allFiles = []
    for dir in dirs:
        ...
        if not file['dup']:
            ...
            if targetFile != None:
                if  os.path.basename(targetFile) == os.path.basename(file['name']):
                    ...
                    log ("WARN: Conflict detected: copying "+file['name']+ ' to '+targetFile)
                shutil.copy2(file['name'],  targetFile)
                copied += 1
            else:        
                log("WARN: Duplicated in destination: "+file['name']+" not copied!")                    
        else:
...
def showHelpAndExit(msg=''):
    print ("photorg - An small photo organizer (v0.4).")
